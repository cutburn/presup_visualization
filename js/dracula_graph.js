...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function(node) {
		...
		var color = '#663399';
		var good = ['positive','gg','gb','gn'];
		var bad = ['negative','bg','bb','bn'];
                var newShape;
		if ($.inArray(node.sentiment, good) != -1)
		{
			if (node.id[0] == 'e')
			{
				color = '#6666ff';
		...
		if ($.inArray(node.sentiment, bad) != -1)
		{
			//color = '#990000';
			if (node.id[0] == 'e')
			{
		...
		if (node.id[0] == 'e')
		{
			newShape = r.rect(-30,-20,60,40).attr({fill: color, stroke: color, "stroke-width": 2});
			/* set DOM node ID */
			newShape.node.id = node.label || node.id;
			shape = r.set().
			    push(newShape).
		...
		{
			//newShape = r.ellipse(0, 0, 30, 20).attr({fill: color, stroke: color, "stroke-width": 1});
			shape = r.set().
			    push(r.text(0, 30, node.label || node.id));
			return shape;
		}
...
Graph.Layout.Static = function(graph, map) {
    this.graph = graph;
    this.map = map;
    this.layout();
};
Graph.Layout.Static.prototype = {
    layout: function() {
        this.layoutPrepare();
        this.layoutCalcBounds();
    },
    
    layoutPrepare: function(map) {
        for (i in this.graph.nodes) {
            var node = this.graph.nodes[i];
            node.layoutPosX = 0;
            node.layoutPosY = 0;
            ...
            var counter = 0;
            for (i in this.map) {
                var node = this.graph.getNode(this.map[i][0]);
                node.layoutPosX = this.map[i][1];
                node.layoutPosY = this.map[i][2];
                counter++;
            }
    },
